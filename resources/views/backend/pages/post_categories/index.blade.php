...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        {{-- @include('backend.pages.post_categories.filter.filter') --}}
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                @forelse($categories as $category)
                    <tr>
                        <td>{{ $category->name }}</td>
                        <td><a href="{{ route('posts.index', ['category_id' => $category->id]) }}">{{ $category->posts_count }}</a></td>
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <form action="{{ route('categories.destroy', $category->id) }}" method="post" id="category-delete-{{ $category->id }}" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
