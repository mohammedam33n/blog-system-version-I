...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        @include('backend.pages.pages.filter.filter')
            ...
            <table class="table table-hover">
                ...
                @forelse($pages as $page)
                    <tr>
                        ...
                        <td>{{ $page->status() }}</td>
                        <td><a href="{{ route('pages.index', ['category_id' => $page->category_id]) }}">{{ $page->category->name }}</a></td>
                        <td>{{ $page->user->name }}</td>
                        <td>{{ $page->created_at->format('d-m-Y h:i a') }}</td>
                        <td>
                            <div class="btn-group">
                                <a href="{{ route('pages.edit', $page->id) }}" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                                ...
                                <form action="{{ route('pages.destroy', $page->id) }}" method="post" id="page-delete-{{ $page->id }}" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
