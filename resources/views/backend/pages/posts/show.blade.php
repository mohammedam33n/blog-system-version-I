    ...
    <div class="card shadow mb-4">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <tbody>
                    <tr>
                        <td colspan="4"><a href="{{ route('posts.show', $post->id) }}">{{ $post->title }}</a></td>
                    </tr>
                    <tr>
                        <th>Comments</th>
                            ...
                            <div class="row">
                                @if($post->media->count() > 0)
                                    @foreach($post->media as $media)
                                        <div class="col-2">
                                            <img src="{{ asset('images/posts/' . $media->file_name) }}" class="img-fluid">
                                        </div>
                                    @endforeach
                                @endif
                            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <form action="{{ route('comments.destroy', $comment->id) }}" method="post" id="comment-delete-{{ $comment->id }}" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
