...
@section('content')
    <div class="card shadow mb-4">
        ...
        @include('backend.pages.posts.filter.filter')
                ...
                <tbody>
                    @forelse($posts as $post)
                        <tr>
                            ...
                            <td>{!! $post->comment_able == 1? "<a href=\"" . route('comments.index', ['post_id' => $post->id]) . "\">" . $post->comments->count() . '</a>': 'Disallow' !!}</td>
                            <td>{{ $post->status() }}</td>
                            <td>
                                <a href="{{ route('posts.index', ['category_id' => $post->category_id]) }}">{{ $post->category->name }}</a>
                            </td>
