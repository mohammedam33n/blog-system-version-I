    ...
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-12">
                <div class="form-group">
                    ...
                    {!! Form::textarea('description', old('description', $post->description), [
        ...
        <div class="row">
            <div class="col-4">
                ...
                {!! Form::select('category_id', ['' => '---'] + $categories->toArray(), old('category_id', $post->category_id), [
                    'class' => 'form-control',
                ]) !!}
                @error('category_id')
                    <span class="text-danger">{{ $message }}</span>
        ...
        $(function () {
            ...
            $('#post-images').fileinput({
                ...
                initialPreview: [
                    @if($post->media->count() > 0)
                        @foreach($post->media as $media)
                            "{{ asset('images/posts/' . $media->file_name) }}",
                        @endforeach
                ...
                initialPreviewConfig: [
                    @if($post->media->count() > 0)
                        @foreach($post->media as $media)
                            {caption: "{{ $media->file_name }}", size: {{ $media->file_size }}, width: "120px", url: "{{ route('posts.media.destroy', [$media->id, '_token' => csrf_token()]) }}", key: "{{ $media->id }}"},
                        @endforeach
                    @endif
                ],
            });
